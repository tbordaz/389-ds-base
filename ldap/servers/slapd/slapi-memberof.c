...
#define MEMBEROF_CACHE_DEBUG 1


typedef struct _Xmemberof_get_groups_data
{
    ...
    PRBool use_cache;
} Xmemberof_get_groups_data;

static int Xmemberof_get_groups_r(Slapi_MemberOfConfig *config, Slapi_DN *member_sdn, Xmemberof_get_groups_data *data);
 ...
 */
typedef struct _Xmemberof_cached_value
{
    char *key;
    char *group_dn_val;
    char *group_ndn_val;
...
static struct Xcache_stat Xcache_stat;

int Xmemberof_compare(Slapi_MemberOfConfig *config, const void *a, const void *b);
static void report_error_msg(Slapi_MemberOfConfig *config, char* msg);

void
Xdump_cache_entry(Xmemberof_cached_value *double_check, const char *msg)
{
    int i;
    for (i = 0; double_check[i].valid; i++) {
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "dump_cache_entry: %s -> %s/%s\n",
                      msg ? msg : "<no key>",
...
static Xmemberof_cached_value *
Xancestors_cache_lookup(Slapi_MemberOfConfig *config, const char *ndn)
{
    Xmemberof_cached_value *e;
#if defined(DEBUG) && defined(HAVE_CLOCK_GETTIME)
    long int start;
    struct timespec tsnow;
#endif

    Xcache_stat.total_lookup++;
    ...
    e = (Xmemberof_cached_value *) PL_HashTableLookupConst(config->ancestors_cache, (const void *) ndn);

#if defined(DEBUG) && defined(HAVE_CLOCK_GETTIME)
    if (start) {
        if (clock_gettime(CLOCK_REALTIME, &tsnow) == 0) {
            Xcache_stat.cumul_duration_lookup += (tsnow.tv_nsec - start);
        }
    }
#endif

    if (e)
        Xcache_stat.successfull_lookup++;
    return e;
}
...
void Xancestor_hashtable_entry_free(Xmemberof_cached_value *entry)
{
    int i;
    ...
    for (i = 0; entry[i].valid; i++) {
        ...
        slapi_ch_free((void **) &entry[i].nsuniqueid_val);
    }
...
static PLHashEntry *
Xancestors_cache_add(Slapi_MemberOfConfig *config, const void *key, void *value)
{
    PLHashEntry *e;
#if defined(DEBUG) && defined(HAVE_CLOCK_GETTIME)
    long int start;
    ...
    if (start) {
        if (clock_gettime(CLOCK_REALTIME, &tsnow) == 0) {
            Xcache_stat.cumul_duration_add += (tsnow.tv_nsec - start);
        }
    }
#endif
    return e;
 ...
 */
static void
Xcache_ancestors(Slapi_MemberOfConfig *config, Slapi_Value **member_ndn_val, Xmemberof_get_groups_data *groups)
{
    Slapi_ValueSet *groupvals = *((Xmemberof_get_groups_data *)groups)->groupvals;
    Slapi_ValueSet *nsuniqueidvals = *((Xmemberof_get_groups_data *)groups)->nsuniqueidvals;
    Slapi_Value *sval_dn, *sval_nsuniqueid;
    Slapi_DN *sdn = 0;
    const char *dn, *nsuniqueid;
    const char *ndn;
    const char *key;
    char *key_copy;
    int hint_dn = 0;
    ...
    int count;
    int index;
    Xmemberof_cached_value *cache_entry;
#if MEMBEROF_CACHE_DEBUG
    Xmemberof_cached_value *double_check;
#endif

    if ((member_ndn_val == NULL) || (*member_ndn_val == NULL)) {
        slapi_log_err(SLAPI_LOG_FATAL, "linked_attributes", "cache_ancestors: Fail to cache groups ancestor of unknown member\n");
        return;
    }
    ...
    if (count == 0) {
         ...
         */
        cache_entry = (Xmemberof_cached_value *)slapi_ch_calloc(2, sizeof(Xmemberof_cached_value));
        if (!cache_entry) {
            slapi_log_err(SLAPI_LOG_FATAL, "linked_attributes", "cache_ancestors: Fail to cache no group are ancestor of %s\n",
...
#if MEMBEROF_CACHE_DEBUG
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "cache_ancestors: For %s cache %s/%s\n",
                      slapi_value_get_string(*member_ndn_val), 
        ...
        cache_entry = (Xmemberof_cached_value *)slapi_ch_calloc(count + 1, sizeof(Xmemberof_cached_value));
        if (!cache_entry) {
            slapi_log_err(SLAPI_LOG_FATAL, "linked_attributes", "cache_ancestors: Fail to cache groups ancestor of %s\n",
                          slapi_value_get_string(*member_ndn_val));
            return;
        }
        ...
        while (sval_dn) {
             ...
             */
            if (Xmemberof_compare(groups->config, member_ndn_val, &sval_dn)) {
...
#if MEMBEROF_CACHE_DEBUG
                slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "cache_ancestors:    %s/%s\n",
                              cache_entry[index].group_dn_val ? cache_entry[index].group_dn_val : "<empty>",
                              cache_entry[index].nsuniqueid_val ? cache_entry[index].nsuniqueid_val : "<empty>");
#endif
                index++;
...
/* Cache the ancestor of member_ndn_val using the
     ...
     */
#if MEMBEROF_CACHE_DEBUG
    Xdump_cache_entry(cache_entry, key);
#endif
    if (Xancestors_cache_add(config, (const void*) key_copy, (void *) cache_entry) == NULL) {
        slapi_log_err( SLAPI_LOG_FATAL, "linked_attributes", "cache_ancestors: Failed to cache ancestor of %s\n", key);
        Xancestor_hashtable_entry_free(cache_entry);
        slapi_ch_free ((void**)&cache_entry);
        return;
    }
#if MEMBEROF_CACHE_DEBUG
    if ((double_check = Xancestors_cache_lookup(config, (const void*) key)) != NULL) {
        Xdump_cache_entry(double_check, "read back");
    }
#endif
}
 ...
 */
int
Xmemberof_compare(Slapi_MemberOfConfig *config, const void *a, const void *b)
{
    Slapi_Value *val1;
    Slapi_Value *val2;
 ...
 */
static void
Xmerge_ancestors(Slapi_Value **member_ndn_val, Xmemberof_get_groups_data *v1, Xmemberof_get_groups_data *v2)
{
    ...
    int hint = 0;
    int hint_nsuniqueid = 0;
    Slapi_MemberOfConfig *config = ((Xmemberof_get_groups_data *)v2)->config;
    Slapi_ValueSet *v1_groupvals = *((Xmemberof_get_groups_data *)v1)->groupvals;
    Slapi_ValueSet *v2_groupvals = *((Xmemberof_get_groups_data *)v2)->groupvals;
    Slapi_ValueSet *v2_group_norm_vals = *((Xmemberof_get_groups_data *)v2)->group_norm_vals;
    Slapi_ValueSet *v1_nsuniqueidvals = *((Xmemberof_get_groups_data *)v1)->nsuniqueidvals;
    Slapi_ValueSet *v2_nsuniqueidvals = *((Xmemberof_get_groups_data *)v2)->nsuniqueidvals;
    int merged_cnt = 0;

    hint = slapi_valueset_first_value(v1_groupvals, &sval);
    hint_nsuniqueid = slapi_valueset_first_value(v1_nsuniqueidvals, &sval_2);
    while (sval) {
        if (Xmemberof_compare(config, member_ndn_val, &sval)) {
            ...
            if (sval_dn) {
                ...
                if (!slapi_valueset_find(
                        ((Xmemberof_get_groups_data *)v2)->config->dn_syntax_attr, v2_group_norm_vals, sval_ndn)) {
/* This ancestor was not already present in v2 => Add it
                                         * Using slapi_valueset_add_value it consumes val
                                         * so do not free sval
                                         */
#if MEMBEROF_CACHE_DEBUG
                    slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "merge_ancestors: add %s\n", slapi_value_get_string(sval_ndn));
#endif
...
#if MEMBEROF_CACHE_DEBUG
                    slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "merge_ancestors: skip (already present) %s\n", slapi_value_get_string(sval_ndn));
#endif
 ...
 */
static int
Xmemberof_entry_in_scope(Slapi_MemberOfConfig *config, Slapi_DN *sdn)
{
 ...
 */
int
Xmemberof_get_groups_callback(Slapi_Entry *e, void *callback_data)
{
    ...
    Slapi_ValueSet *groupvals = *((Xmemberof_get_groups_data *)callback_data)->groupvals;
    Slapi_ValueSet *group_norm_vals = *((Xmemberof_get_groups_data *)callback_data)->group_norm_vals;
    Slapi_ValueSet *group_nsuniqueid_vals = *((Xmemberof_get_groups_data *)callback_data)->nsuniqueidvals;
    Slapi_ValueSet *already_seen_ndn_vals = *((Xmemberof_get_groups_data *)callback_data)->already_seen_ndn_vals;
    Slapi_MemberOfConfig *config = ((Xmemberof_get_groups_data *)callback_data)->config;
    int rc = 0;
    ...
    if (!groupvals || !group_norm_vals) {
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes",
    ...
    if (0 == Xmemberof_compare(((Xmemberof_get_groups_data *)callback_data)->config,
                              &((Xmemberof_get_groups_data *)callback_data)->memberdn_val, &group_ndn_val)) {
        ...
        ((Xmemberof_get_groups_data *)callback_data)->use_cache = PR_FALSE;
        goto bail;
    }
    ...
    if (slapi_valueset_find(
            ((Xmemberof_get_groups_data *)callback_data)->config->dn_syntax_attr, already_seen_ndn_vals, group_ndn_val)) {
    ...
    if (Xmemberof_entry_in_scope(config, group_sdn)) {
        ...
        if ((config->maxgroups) && 
            ...
            char *msg = "slapi_memberof: result set truncated because of maxgroups limit (from computation)";
            report_error_msg(config, msg);
    ...
    if (config->recurse && (config->maxgroups_reached == PR_FALSE)) {
        ...
        Xmemberof_get_groups_r(((Xmemberof_get_groups_data *)callback_data)->config,
...
static void
report_error_msg(Slapi_MemberOfConfig *config, char* msg)
{
    int32_t len;
...
static int
entry_get_groups(Slapi_MemberOfConfig *config, Slapi_DN *member_sdn, char *memberof_attr, Slapi_ValueSet *groupvals, Slapi_ValueSet *nsuniqueidvals)
{
    ...
    if (rc != LDAP_SUCCESS || member_entry == NULL) {
        ...
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes",
                      "entry_get_groups - Failed to retrieve target entry %s: %d\n",
    ...
    attrs[0] = "nsuniqueid";
    attrs[1] = NULL;
    for (int32_t i = 0; groups_dn && groups_dn[i]; i++) {
...
static PRBool
compare_memberof_config(const char *memberof_attr, char **groupattrs, PRBool all_backends, PRBool skip_nested, Slapi_DN **include_scope, Slapi_DN **exclude_scope, PRBool enabled_only)
{
    int32_t cnt1, cnt2;
    
    if ((memberof_config == NULL) || (memberof_config->enabled == PR_FALSE)) {
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: config not initialized or disabled\n");
        return PR_FALSE;
    }
    
    if (enabled_only) {
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: check the plugin is enabled that is %s\n",
    ...
    if ((all_backends != memberof_config->all_backends) || (skip_nested != memberof_config->skip_nested)) {
        ...
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails (allbackend %d vs %d, skip_nested %d vs %d)\n",
    ...
    if ((memberof_attr == NULL) || (memberof_config->memberof_attr == NULL) || (strcasecmp(memberof_attr, memberof_config->memberof_attr))) {
        ...
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes",
                     "compare_memberof_config: fails memberof attribute differs (require %s vs config %s)\n",
    ...
    if (groupattrs == NULL) {
        ...
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because requested group attributes is empty\n");
        return PR_FALSE;
    }
    for (cnt1 = 0; groupattrs[cnt1]; cnt1++) {
        if (charray_inlist(memberof_config->groupattrs, groupattrs[cnt1]) == 0) {
            slapi_log_err(SLAPI_LOG_ERR, "linked_attributes",
                          "compare_memberof_config: fails because requested group attribute %s is not configured\n",
                          groupattrs[cnt1]);
            return PR_FALSE;
        }
    }
    ...
    if (cnt1 != cnt2) {
        ...
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because number of requested group attributes differs from config\n");
        return PR_FALSE;
    }
    ...
    if (include_scope == NULL) {
        if (memberof_config->include_scope) {
            slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because requested include scope is empty that differs config\n");
            return PR_FALSE;
        }
    } else {
        if (memberof_config->include_scope == NULL) {
            slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because requested include scope is not empty that differs config\n");
            return PR_FALSE;
        }
    }
    ...
    for (cnt1 = 0; include_scope && include_scope[cnt1]; cnt1++) {
        if (charray_inlist(memberof_config->include_scope, (char *) slapi_sdn_get_ndn(include_scope[cnt1])) == 0) {
            slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because requested include scope (%s) is not in config\n",
    ...
    if (cnt1 != cnt2) {
        ...
        slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because number of requested included scopes differs from config\n");
        return PR_FALSE;
    }
    ...
    if (exclude_scope == NULL) {
        if (memberof_config->exclude_scope) {
            slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because requested exclude scope is empty that differs config\n");
            return PR_FALSE;
        }
    } else {
        if (memberof_config->exclude_scope == NULL) {
            slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because requested exclude scope is not empty that differs config\n");
            return PR_FALSE;
        }
    }
    ...
    for (cnt1 = 0; exclude_scope && exclude_scope[cnt1]; cnt1++) {
        if (charray_inlist(memberof_config->exclude_scope, (char *) slapi_sdn_get_ndn(exclude_scope[cnt1])) == 0) {
            slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: fails because requested exclude scope (%s) is not in config\n",
    ...
    slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "compare_memberof_config: succeeds. requested options match config\n");
    return PR_TRUE;
}

static void
Xadd_ancestors_cbdata(Xmemberof_cached_value *ancestors, void *callback_data)
{
    Slapi_Value *sval;
    int val_index;
    Slapi_ValueSet *group_norm_vals_cbdata = *((Xmemberof_get_groups_data *)callback_data)->group_norm_vals;
    Slapi_ValueSet *group_vals_cbdata = *((Xmemberof_get_groups_data *)callback_data)->groupvals;
    Slapi_ValueSet *nsuniqueid_vals_cbdata = *((Xmemberof_get_groups_data *)callback_data)->nsuniqueidvals;
    Slapi_Value *memberdn_val = ((Xmemberof_get_groups_data *)callback_data)->memberdn_val;
    int added_group;
    ...
    for (val_index = 0, added_group = 0; ancestors[val_index].valid; val_index++) {
        ...
        if (sval) {
            if (!slapi_valueset_find(
                    ((Xmemberof_get_groups_data *)callback_data)->config->dn_syntax_attr, group_norm_vals_cbdata, sval)) {
    ...
    slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "add_ancestors_cbdata: Ancestors of %s contained %d groups. %d added. %s\n",
...
static Slapi_DN *
memberof_scope_is_child_of_dn(Slapi_MemberOfConfig *config, Slapi_DN *sdn)
{
    int i = 0;
 ...
 */
int
Xmemberof_call_foreach_dn(Slapi_PBlock *pb __attribute__((unused)), Slapi_DN *sdn, Slapi_MemberOfConfig *config, char **types, plugin_search_entry_callback callback, void *callback_data, int *cached, PRBool use_grp_cache)
{
    ...
    *cached = 0;
    ...
    if (!Xmemberof_entry_in_scope(config, sdn)) {
        return (rc);
    }
    ...
    if (use_grp_cache) {
         ...
         * this value
         */
        Xmemberof_cached_value *ht_grp = NULL;
        const char *ndn = slapi_sdn_get_ndn(sdn);

        ht_grp = Xancestors_cache_lookup(config, (const void *)ndn);
        if (ht_grp) {
#if MEMBEROF_CACHE_DEBUG
            slapi_log_err(SLAPI_LOG_PLUGIN, "linked_attributes", "memberof_call_foreach_dn: Ancestors of %s already cached (%p)\n", ndn, ht_grp);
#endif
            Xadd_ancestors_cbdata(ht_grp, callback_data);
            *cached = 1;
            return (rc);
        }
    }
#if MEMBEROF_CACHE_DEBUG
    slapi_log_err(SLAPI_LOG_PLUGIN, "linked_attributes", "memberof_call_foreach_dn: Ancestors of %s not cached\n", slapi_sdn_get_ndn(sdn));
#endif
    ...
    for (i = 0; (types[i] && (config->maxgroups_reached == PR_FALSE)); i++) {
        ...
        while ((config->maxgroups_reached == PR_FALSE) && be) {
            ...
            if ((config->entryScopes && config->entryScopes[0]) ||
                ...
                if (Xmemberof_entry_in_scope(config, base_sdn)) {
...
static int
Xmemberof_get_groups_r(Slapi_MemberOfConfig *config, Slapi_DN *member_sdn, Xmemberof_get_groups_data *data)
{
    ...
    Xmemberof_get_groups_data member_data = {config, member_ndn_val, &groupvals, &group_norm_vals, &nsuniqueidvals, data->already_seen_ndn_vals, data->use_cache};

/* Search for any grouping attributes that point to memberdn.
     * For each match, add it to the list, recurse and do same search */
#if MEMBEROF_CACHE_DEBUG
    slapi_log_err(SLAPI_LOG_ERR, "linked_attributes", "memberof_get_groups_r: Ancestors of %s\n", slapi_sdn_get_dn(member_sdn));
#endif
    rc = Xmemberof_call_foreach_dn(NULL, member_sdn, config, config->groupattrs,
                                  Xmemberof_get_groups_callback, &member_data, &cached, member_data.use_cache);

    Xmerge_ancestors(&member_ndn_val, &member_data, data);
    if (!cached && member_data.use_cache)
        Xcache_ancestors(config, &member_ndn_val, &member_data);
...
static PRIntn Xmemberof_hash_compare_keys(const void *v1, const void *v2)
{
    PRIntn rc;
...
static PRIntn Xmemberof_hash_compare_values(const void *v1, const void *v2)
{
    PRIntn rc;
    if ((char *) v1 == (char *) v2) {
        rc = 1;
 ...
 */
static PLHashNumber Xmemberof_hash_fn(const void *key)
{
...
PLHashTable *hashtable_new(int usetxn)
{
    ...
    return PL_NewHashTable(1000,
        Xmemberof_hash_fn,
        Xmemberof_hash_compare_keys,
        Xmemberof_hash_compare_values, NULL, NULL);
}

/* this function called for each hash node during hash destruction */
static PRIntn Xancestor_hashtable_remove(PLHashEntry *he, PRIntn index __attribute__((unused)), void *arg __attribute__((unused)))
{
    Xmemberof_cached_value *group_ancestor_array;

    if (he == NULL) {
        return HT_ENUMERATE_NEXT;
    }
    group_ancestor_array = (Xmemberof_cached_value *) he->value;
    Xancestor_hashtable_entry_free(group_ancestor_array);
    slapi_ch_free((void **)&group_ancestor_array);

    return HT_ENUMERATE_REMOVE;
}

static void Xancestor_hashtable_empty(Slapi_MemberOfConfig *config, char *msg)
{
    if (config->ancestors_cache) {
        PL_HashTableEnumerateEntries(config->ancestors_cache, Xancestor_hashtable_remove, msg);
    }
}
...
slapi_memberof(Slapi_MemberOfConfig *config, Slapi_DN *member_sdn, Slapi_MemberOfResult *result)
{
    ...
    if ((config->flag == MEMBEROF_REUSE_ONLY) && compare_memberof_config(NULL,
                                                                NULL,
                                                                PR_FALSE,
                                                                PR_FALSE,
                                                                NULL,
         ...
         */
        rc = entry_get_groups(config, member_sdn, "memberof", groupvals, nsuniqueidvals);
    } else if ((config->flag == MEMBEROF_REUSE_IF_POSSIBLE) && compare_memberof_config("memberof",
                                                                              config->groupattrs,
                                                                              config->allBackends,
                                                                              !config->recurse,
                                                                              config->entryScopes,
        ...
        config->ancestors_cache = hashtable_new(1);
        Xmemberof_get_groups_data data = {config, memberdn_val, &groupvals, &group_norm_vals, &nsuniqueidvals, &already_seen_ndn_vals, PR_TRUE};

        rc = Xmemberof_get_groups_r(config, member_sdn, &data);
        ...
        if (config->ancestors_cache) {
            Xancestor_hashtable_empty(config, "memberof_free_config empty group_ancestors_hashtable");
